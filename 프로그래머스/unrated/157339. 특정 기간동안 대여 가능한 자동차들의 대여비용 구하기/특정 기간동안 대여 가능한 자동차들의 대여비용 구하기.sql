SELECT A.CAR_ID, A.CAR_TYPE, ROUND(A.DAILY_FEE-(A.DAILY_FEE * B.DISCOUNT_RATE/100))*30 AS FEE
FROM(
SELECT  A.CAR_ID, A.CAR_TYPE, A.DAILY_FEE, MAX(B.START_DATE), MAX(B.END_DATE)
FROM CAR_RENTAL_COMPANY_CAR A INNER JOIN CAR_RENTAL_COMPANY_RENTAL_HISTORY B
                            ON A.CAR_ID = B.CAR_ID
WHERE A.CAR_TYPE IN ('세단','SUV')
       # AND B.END_DATE <= "2022-11-01"
       # OR B.START_DATE >= "2022-12-01 00:00:00"
GROUP BY A.CAR_ID
HAVING MAX(B.END_DATE) < "2022-11-01") A 
